...
ML_TRACKING_MIDDLEWARE_ENABLED = ML_ENABLED and _env_bool('ML_TRACKING_MIDDLEWARE_ENABLED', True)
ML_PRODUCT_TRACKING_MIDDLEWARE_ENABLED = ML_ENABLED and _env_bool('ML_PRODUCT_TRACKING_MIDDLEWARE_ENABLED', True)
ML_AUTO_TRAINING = ML_ENABLED and _env_bool('ML_AUTO_TRAINING', True)
ML_AUTO_RETRAIN_ON_DRIFT = ML_ENABLED and _env_bool('ML_AUTO_RETRAIN_ON_DRIFT', True)
ML_TRACK_ADMIN = _env_bool('ML_TRACK_ADMIN', True)
...
CHAT_AI_ALLOWED_MODELS = os.environ.get(
    'CHAT_AI_ALLOWED_MODELS',
    'Qwen/*,microsoft/Phi-3.5-mini-instruct,mistralai/Ministral-8B-Instruct-2410,meta-llama/Llama-3.2-3B-Instruct',
).strip()
CHAT_AI_MODEL_FALLBACKS = os.environ.get(
    'CHAT_AI_MODEL_FALLBACKS',
    'Qwen/Qwen2.5-0.5B-Instruct,microsoft/Phi-3.5-mini-instruct',
).strip()
CHAT_AI_BLOCKED_MODELS = os.environ.get('CHAT_AI_BLOCKED_MODELS', '').strip()
...
CHAT_AI_GENERATION_MAX_TIME_SECONDS = _env_float('CHAT_AI_GENERATION_MAX_TIME_SECONDS', 12.0)
CHAT_AI_SERIALIZE_GENERATION = _env_bool('CHAT_AI_SERIALIZE_GENERATION', True)
CHAT_AI_USE_KV_CACHE = _env_bool('CHAT_AI_USE_KV_CACHE', True)
...
CHAT_AI_INCLUDE_ORDER_CONTEXT = _env_bool('CHAT_AI_INCLUDE_ORDER_CONTEXT', True)
CHAT_AI_INCLUDE_ML_PROFILE = _env_bool('CHAT_AI_INCLUDE_ML_PROFILE', True)
CHAT_AI_INCLUDE_CANNED_KNOWLEDGE = _env_bool('CHAT_AI_INCLUDE_CANNED_KNOWLEDGE', True)
CHAT_AI_KNOWLEDGE_CANDIDATE_LIMIT = _env_int('CHAT_AI_KNOWLEDGE_CANDIDATE_LIMIT', 20)
CHAT_AI_KNOWLEDGE_SNIPPET_LIMIT = _env_int('CHAT_AI_KNOWLEDGE_SNIPPET_LIMIT', 3)
CHAT_AI_KNOWLEDGE_SNIPPET_MAX_CHARS = _env_int('CHAT_AI_KNOWLEDGE_SNIPPET_MAX_CHARS', 240)
CHAT_AI_MAX_PERSONALIZATION_FIELDS = _env_int('CHAT_AI_MAX_PERSONALIZATION_FIELDS', 16)
...
# Product AI autofill configuration
PRODUCT_AI_ENABLED = _env_bool('PRODUCT_AI_ENABLED', True)
PRODUCT_AI_CONFIDENCE_THRESHOLD = _env_float('PRODUCT_AI_CONFIDENCE_THRESHOLD', 0.80)
PRODUCT_AI_MAX_IMAGES = _env_int('PRODUCT_AI_MAX_IMAGES', 4)
PRODUCT_AI_MAX_IMAGE_SIZE_MB = _env_int('PRODUCT_AI_MAX_IMAGE_SIZE_MB', 8)
PRODUCT_AI_MAX_CONCURRENT_JOBS = _env_int('PRODUCT_AI_MAX_CONCURRENT_JOBS', 2)
...
PRODUCT_AI_STRICT_EVIDENCE_HARD_FAIL = _env_bool('PRODUCT_AI_STRICT_EVIDENCE_HARD_FAIL', False)
...
PRODUCT_AI_MIN_WEB_SOURCES_WITH_NN = _env_int('PRODUCT_AI_MIN_WEB_SOURCES_WITH_NN', 1)
PRODUCT_AI_MIN_HIGH_TRUST_DOCS = _env_int('PRODUCT_AI_MIN_HIGH_TRUST_DOCS', 1)
PRODUCT_AI_STRICT_REQUIRE_HIGH_TRUST = _env_bool('PRODUCT_AI_STRICT_REQUIRE_HIGH_TRUST', False)
...
PRODUCT_AI_NN_ENABLED = _env_bool('PRODUCT_AI_NN_ENABLED', True)
PRODUCT_AI_NN_MODEL_ID = os.environ.get('PRODUCT_AI_NN_MODEL_ID', 'openai/clip-vit-base-patch32').strip()
PRODUCT_AI_NN_DEVICE = os.environ.get('PRODUCT_AI_NN_DEVICE', 'cpu').strip().lower()
PRODUCT_AI_NN_TIMEOUT_SECONDS = _env_float('PRODUCT_AI_NN_TIMEOUT_SECONDS', 8.0)
PRODUCT_AI_NN_CONFIDENCE_THRESHOLD = _env_float('PRODUCT_AI_NN_CONFIDENCE_THRESHOLD', 0.26)
PRODUCT_AI_NN_MODEL_CACHE_DIR = os.environ.get(
    'PRODUCT_AI_NN_MODEL_CACHE_DIR',
    str(BASE_DIR / 'ml' / 'models_data' / 'catalog_product_ai'),
).strip()
...
PRODUCT_AI_RESEARCH_RESPECT_ROBOTS = _env_bool('PRODUCT_AI_RESEARCH_RESPECT_ROBOTS', False)
PRODUCT_AI_RESEARCH_ALLOW_SNIPPET_FALLBACK = _env_bool(
    'PRODUCT_AI_RESEARCH_ALLOW_SNIPPET_FALLBACK',
    True,
)
PRODUCT_AI_RESEARCH_MIN_EXTRACTED_CHARS = _env_int('PRODUCT_AI_RESEARCH_MIN_EXTRACTED_CHARS', 80)
PRODUCT_AI_USER_AGENT = os.environ.get('PRODUCT_AI_USER_AGENT', 'Mozilla/5.0').strip()
PRODUCT_AI_SEARCH_USER_AGENT = os.environ.get(
    'PRODUCT_AI_SEARCH_USER_AGENT',
    PRODUCT_AI_USER_AGENT or 'Mozilla/5.0',
).strip()
PRODUCT_AI_RESEARCH_USER_AGENT = os.environ.get(
    'PRODUCT_AI_RESEARCH_USER_AGENT',
    PRODUCT_AI_USER_AGENT or 'Mozilla/5.0',
).strip()
...
PRODUCT_AI_DEEP_RESEARCH_MAX_RESULTS_PER_QUERY = _env_int(
    'PRODUCT_AI_DEEP_RESEARCH_MAX_RESULTS_PER_QUERY',
    6,
)
PRODUCT_AI_DEEP_RESEARCH_MAX_SEARCH_RESULTS = _env_int('PRODUCT_AI_DEEP_RESEARCH_MAX_SEARCH_RESULTS', 24)
PRODUCT_AI_DEEP_RESEARCH_MAX_DOCS = _env_int('PRODUCT_AI_DEEP_RESEARCH_MAX_DOCS', 10)
PRODUCT_AI_DEEP_RESEARCH_MAX_SOURCES = _env_int('PRODUCT_AI_DEEP_RESEARCH_MAX_SOURCES', 8)
PRODUCT_AI_DEEP_RESEARCH_MAX_DOMAIN_REPEATS = _env_int(
    'PRODUCT_AI_DEEP_RESEARCH_MAX_DOMAIN_REPEATS',
    2,
)
PRODUCT_AI_DEEP_RESEARCH_MIN_SCORE = _env_float('PRODUCT_AI_DEEP_RESEARCH_MIN_SCORE', 0.20)
PRODUCT_AI_MARKETPLACE_DOMAINS = _env_csv(
    'PRODUCT_AI_MARKETPLACE_DOMAINS',
    'amazon.com,etsy.com,ebay.com,walmart.com,target.com,aliexpress.com,flipkart.com,aarong.com,daraz.com.bd,arjobd.com',
)
PRODUCT_AI_VISION_MODEL = os.environ.get(
    'PRODUCT_AI_VISION_MODEL',
...
if PRODUCT_AI_NN_ENABLED:
    Path(PRODUCT_AI_NN_MODEL_CACHE_DIR).mkdir(parents=True, exist_ok=True)
